...
char *DefaultDevice()
{
  ...
  for (i = 1; (scsiDevice = (io_object_t) IOIteratorNext(scsiObjectIterator)) != (io_object_t) 0; i++)  {
  ...
  for (i = 1; (scsiDevice = IOIteratorNext(scsiObjectIterator)) != (io_object_t) 0; i++)  {
    deviceName = g_malloc0(80);
...
DeviceHandle* OpenDevice(char *device)
{
  ...
  if (scsiObjectIterator == (io_iterator_t) 0) {
    g_free(dh);
